...
Public Class sys_interproc
  ...
  Protected Overrides Sub WndProc(ByRef m As System.Windows.Forms.Message)
    MyBase.WndProc(m)
    'Stop
    Dim myMsg As WndMsg = m.Msg
    'Debug.Print(winTitle + "     " + myMsg.ToString)
    'Stop
    If m.Msg = WndMsg.WM_COPYDATA Then
      'Stop

      ' kopieren in struct ist langsam und unnoetig:
      'Dim CD As New COPYDATASTRUCT
      'CD = Marshal.PtrToStructure(m.LParam, CD.GetType)

      ' direkt von Offset lesen geht schneller:
      Dim cdData As Integer = Marshal.ReadInt32(m.LParam, 4)
      Dim lpData As Integer = Marshal.ReadInt32(m.LParam, 8)

      Dim strDest = Marshal.PtrToStringUni(lpData, cdData / 2)
      'Dim strDest As New String(" "c, cdData / 2)
      
      'Dim hgcDest As GCHandle = _
      'GCHandle.Alloc(strDest, GCHandleType.Pinned)

      'Call InterprocApi.CopyMemory(hgcDest.AddrOfPinnedObject.ToInt32, lpData, cdData)
      'hgcDest.Free()
      ...
      Select Case parts(0)
        Case "DGET "
          ...
          If commandDef.ContainsKey("QUERY:" + cmdName) = False Then
            trace("sys_interproc - ERROR: unknown query: " + strDest)
        ...
        Case "CMD  "
          If commandDef.ContainsKey("CMD:" + cmdName) = False Then
            trace("sys_interproc - ERROR: unknown command: " + strDest)
          Else
  ...
  Function GetData(ByVal titleName As String, ByVal cmdString As String, ByVal para As String, Optional ByVal timeout As Integer = 5000) As String
    ...
    trace("sys_interproc - FetchData:", titleName, cmdString)

    Do
      idle()
      If myticks = responseTick AndAlso cmdString = responseCmd Then
        trace("sys_interproc - FetchData:", "RESULT=" + responseData)
        Return responseData
      End If
      If myticks + (timeout * 10) < Now.Ticks Then trace("sys_interproc - FetchData:", "TIMEOUT") : Return ""
    Loop
  End Function
  ...
  Function SendDataTitle(ByVal titleName As String, ByVal cmdType As String, ByVal cmdString As String, ByVal para As String, ByVal ticks As Long) As Boolean
    ...
    If hwnd = IntPtr.Zero Then
      Return False
    End If
    trace("sys_interproc - SendDataTitle:", titleName + "/" + hwnd.ToString, cmdType, cmdString, para)
  ...
  Function SendDataHwnd(ByVal hwnd As IntPtr, ByVal cmdType As String, ByVal cmdString As String, ByVal para As String, ByVal ticks As Long) As Boolean
    Try
      ...
      InterprocApi.SendMessage(New HandleRef(Me, hwnd), WndMsg.WM_COPYDATA, Me.Handle, structPointer)
  ...
  Sub EnsureAppRunning(ByVal winTitle As String, Optional ByVal appName As String = "", Optional ByVal appFileSpec As String = "")
    If getWindow(winTitle.ToLower) = IntPtr.Zero Then
      ...
      If appFileSpec = "" Then
        ...
        ElseIf IO.File.Exists(glob.appPath + appName + ".exe") Then
          appFileSpec = glob.appPath + appName + ".exe"
        End If
      End If
  ...
  Private Function EnumWindowsProc(ByVal hwnd As IntPtr, ByVal lParam As Integer) As Integer
    ...
    If title.StartsWith("sys_interproc@") Then
      'Stop
      trace("sys_interproc - window Found:", ">" + title + "<")
