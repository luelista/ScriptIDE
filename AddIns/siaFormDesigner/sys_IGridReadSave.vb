...
Module sys_IGridReadSave
  ...
  Sub SplitToIGridLine(ByVal line As iGRow, ByVal input As String, Optional ByVal Delimiter As String = vbTab, Optional ByVal autoAddCols As Boolean = False)
    Dim max = line.Cells.Count - 1
    Dim out() = Split(input, Delimiter)
    If autoAddCols AndAlso out.Length > max Then line.Grid.Cols.Count = out.Length : max = out.Length - 1
  ...
  End Sub
  Sub Igrid_put(ByVal Grid As iGrid, ByVal FileContent As String, Optional ByVal LineDelim As String = vbNewLine, Optional ByVal ColDelim As String = vbTab, Optional ByVal autoAddCols As Boolean = False)
    Dim out() = Split(FileContent, LineDelim)
    Grid.Rows.Clear()
    Grid.Rows.Count = out.Length
    For i As Integer = 0 To out.Length - 1
      SplitToIGridLine(Grid.Rows(i), out(i), ColDelim, autoAddCols)
    Next
  End Sub
