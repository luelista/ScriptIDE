...
<Microsoft.VisualBasic.ComClass()> Public Class cls_fileSearcher
  ...
  Private ff_fileCount, ff_folderCount, ff_elapsed As Integer, ff_actFolder As String
  Private out() As String, outIdx, outMaxIdx As Integer

  'Private p_scriptClass As String, clsCallback As Object
  Private callBack As FileFinderStatusDelegate
  ...
  Public Delegate Sub FileFinderStatusDelegate(ByVal FolderCount As Integer, ByVal FileCount As Integer, ByVal ActFolder As String, ByVal ActFileSpec As String, ByRef Cancel As Boolean)

  Public Property CallbackScriptClassFunc() As FileFinderStatusDelegate
    Get
      Return callBack
    End Get
    Set(ByVal value As FileFinderStatusDelegate)
      callBack = value
    End Set
  End Property
  ...
  Private Function onStatusEvent(ByVal myActFolder As String, ByVal myActFile As String) As Boolean
    ...
    If callBack IsNot Nothing Then
      callBack.Invoke(ff_folderCount, ff_fileCount, myActFolder, myActFile, cancel)
    End If
  ...
  Sub ResetVars()
    ff_fileCount = 0 : ff_folderCount = 0 : outIdx = 0 : outMaxIdx = 0
  ...
  Sub recursiveFileSearch(ByVal startFolder As String, ByVal rootFolder As String, ByVal fileFilter As String, ByVal findText As String, ByVal recursiv As Boolean)
    ...
    hFind = FindFirstFile(FP(rootFolder + startFolder, fileFilter), wfd)
    Do
      If wfd.cFileName = "." Or wfd.cFileName = ".." Then Continue Do
      Dim relFileSpec As String = FP(startFolder, wfd.cFileName)
      Dim fileSpec As String = FP(rootFolder, relFileSpec)
      Dim fileSpecUpper = fileSpec.ToUpper
      Dim fileNameUpper = wfd.cFileName.ToUpper
      ' Debug.Print(fileSpec & outIdx & "  " & outMaxIdx)
      If outIdx Mod 347 = 0 Then onStatusEvent(startFolder, fileSpec)
